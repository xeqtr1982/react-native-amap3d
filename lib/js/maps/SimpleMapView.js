...
export default class SimpleMapView extends Component<any> {
  ...
  /**
   *
   * @param {object:{maptype:'standard'}} target
   */
  ...
  addTestPoint(target: object, testStatus: string = "STOPPED") {
    this.sendCommand("addTestPoint", [target, testStatus]);
  }

  addTestPoints(target: Array, testStatus: string = "RUNNING") {
    this.sendCommand("addTestPoints", [target, testStatus]);
  }

  changeTestPointsVisible(visible: boolean) {
    this.sendCommand("changeTestPointVisible", [visible]);
  }

  changeRenderField(field: string, ranges: Array) {
    this.sendCommand("changeRenderField", [field, ranges]);
  }

  addCells(cells: Array, size: number = 72) {
      console.log("addCells",cells);
      
    if (cells && cells.length > 0) this.sendCommand("addCells", [cells, size]);
  }

  removeCells() {
    this.sendCommand("removeCells");
  }

  changeCellsVisible(visible: boolean) {
    this.sendCommand("changeCellsVisible", [visible]);
  }
