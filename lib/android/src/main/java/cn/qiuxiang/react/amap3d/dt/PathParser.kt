 ...
 * https://www.programcreek.com/java-api-examples/?code=harjot-oberai/VectorMaster/VectorMaster-master/vectormaster/src/main/java/com/sdsmdg/harjot/vectormaster/utilities/legacyparser/PathParser.java
 */
class PathParser {
...
//        }
    ...
    class PathDataNode {
         ...
         */
        fun interpolatePathDataNode(nodeFrom: PathDataNode,
            ...
            private fun addCommand(path: Path, current: FloatArray,
                ...
                while (k < `val`!!.size) {
                    when (cmd) {
                        ...
                        -> if ((previousCmd == 'M' || previousCmd == 'm') && `val`[k + 0] == 0f && `val`[k + 1] == 0f) {
                            path.addCircle(`val`[k + 0], `val`[k + 1], 1f, Path.Direction.CW)
                        } else {
                            path.rLineTo(`val`[k + 0], `val`[k + 1])
                            currentX += `val`[k + 0]
                            currentY += `val`[k + 1]
